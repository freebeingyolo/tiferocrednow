...
public class NetworkChangeUtil {
    ...
    private NetworkChangeUtil(Context context) {
        mContext = context.getApplicationContext();
    }

    public static NetworkChangeUtil getInstance(Context context) {
        if (instance == null) {
            instance = new NetworkChangeUtil(context);
        }
        return instance;
    }
    ...
    public class NetchangeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, @NonNull Intent intent) {
            if (ConnectivityManager.CONNECTIVITY_ACTION.equals(intent.getAction())) {
                if (!NetworkChangeUtil.isNetworkAvailable(mContext)) {
                    if (mListener != null) {
                        mListener.onNetworkAvailable(false);
                    }
                }else {
                    if (mListener != null) {
                        mListener.onNetworkAvailable(true);
                    }
                }
